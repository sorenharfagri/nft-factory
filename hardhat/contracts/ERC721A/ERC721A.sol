...
contract ERC721A is IERC721A {
    ...
    function mintMany(bytes calldata holders) public {
        require(holders.length % 20 == 0, "Invalid address array length");

        uint256 numOfAddresses = holders.length / 20;
        uint256 i = 0;
        ...
        do {
            assembly {
                to := calldataload(add(56, mul(i, 20)))
                i := add(i, 1)
            }
            ...
            unchecked {
                ...
                // uint256 flag1 = _nextInitializedFlag(quantity);
                // uint256 flag2 = _nextExtraData(zeroAddress, to, 0);

                // console.log("_nextInitializedFlag", flag1);
                // console.log("_nextExtraData", flag2);
                ...
                _packedAddressData[to] += 18446744073709551617;
                // Mask `to` to the lower 160 bits, in case the upper bits somehow aren't clean.
                uint256 toMasked = uint256(uint160(to)) & _BITMASK_ADDRESS;
